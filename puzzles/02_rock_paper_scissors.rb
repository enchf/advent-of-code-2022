...
selected: Rock defeats Scissors, Scissors defeats Paper, and Paper defeats Rock. If both players choose the same shape,
...
SHAPES = [
    ['A', 'Z', 'X', 'Y'], # Rock wins against Scissors
    ['B', 'X', 'Y', 'Z'], # Paper wins against Rock
    ['C', 'Y', 'Z', 'X']  # Scissors wins against Paper
]
VALUES = %w[X Y Z]

def result(a, b)
    i = SHAPES.index { |shape, *_| shape == a }
    _, lose, draw, _ = SHAPES[i]

    shape_value = VALUES.index(b) + 1 
    game_result = b == lose ? 0 : b == draw ? 3 : 6

    shape_value + game_result
end

inputs = File
     .readlines('puzzles/02_rock_paper_scissors.in')
     .map { |line| line.split(' ') }
...
puts inputs
        .map { |a, b| [a, VALUES.index(b) + 1] }
        .map { |a, b| result(a, SHAPES[SHAPES.index { |shape, *_| shape == a }][b]) }
        .sum
