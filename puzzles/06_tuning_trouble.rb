...
jpqm, which are all different. In this case, your subroutine should report the value 7, because the first start-of-packet marker
...
def find_marker(line, size)
    (size...line.length).find { |i| line[i - size...i].chars.uniq.size == size }
end

all_lines = File.readlines('puzzles/06_tuning_trouble.in') + File.readlines('puzzles/06_tuning_trouble_samples.in')

all_lines.each { |line| puts find_marker(line, 4) }
...
puts 'Part II'
all_lines.each { |line| puts find_marker(line, 14) }
