...
F = Struct.new(:name, :parent, :size)
D = Struct.new(:name, :parent, :entries) do
    def size
        entries.values.map(&:size).sum
    end
...
def scan_tree(entry, acc = [], &predicate)
    return if entry.is_a?(F)

    entry.entries.values.each { |entry| scan_tree(entry, acc, &predicate) }
    entry.size.tap { |size| acc << size if predicate.(size) }
end

def cd(line)
    command_size = 5 # '$ cd '
    line[command_size..].strip
end

File.readlines('puzzles/07_no_space_left_on_device.in')
    .map(&:strip)
    .each do |line|
        if line.start_with?('$ cd')
            dir = cd(line)
            currdir = dir == '..' ? currdir.parent : currdir.nil? ? D.new(dir, nil, {}).tap { |dir| root = dir } : currdir.get(dir)
        elsif line.start_with?('$ ls')
            next
        elsif line.start_with?('dir')
            name = line[4..].strip # 'dir '
            newdir = D.new(name, currdir, {})
            currdir.add(name, newdir)
        else
            size, name = line.split(' ')
            newfile = F.new(name, currdir, size.to_i)
            currdir.add(name, newfile)
        end
    end

list = []
scan_tree(root, list) { |size| size <= MAX_SIZE }
puts list.sum
...
space_free   = DISK_SPACE - used_space
space_needed = UPDATE_SPACE - space_free

other_list = []
scan_tree(root, other_list) { |size| size >= space_needed }
puts other_list.min
