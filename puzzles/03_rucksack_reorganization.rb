...
def chr_value(chr)
    extra = ('a'..'z').include?(chr) ? 0 : 26
    val = chr.upcase.ord
    extra + val - 'A'.ord + 1
end

puts File.readlines('puzzles/03_rucksack_reorganization.in')
    .map { |line| [line[0...line.length/2], line[line.length/2...line.length]] }
    .map { |left, right| left.chars & right.chars }
    .map(&:first)
    .map { |chr| chr_value(chr) }
    .sum
