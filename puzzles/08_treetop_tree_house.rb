...
def print_grid(grid)
  grid.map { |row| "[ #{ row.map { |h, i, j| "[#{h} (#{i}, #{j})]" }.join(', ') } ]" }.join("\n")
end

visibles = Set.new
grid = File.readlines('puzzles/08_treetop_tree_house.in')
           .map(&:strip)
           .each_with_index
           .map { |line, i| line.chars.map(&:to_i).each_with_index.map { |height, j| [height, i, j] }.to_a }
           .to_a

# Rotate the grid 360 and in each rotation count the visible trees.
(1..4).each do
  puts "Starting point: #{grid.first.first}\n"
